...
假设投票人手里拿着选票排好队:主持人开始收选票：主持人手里只拿一个人的选票：主持人挨个收选票：
...
如果题目不保证一定有一个元素出现的次数多余1/2,那此算法失效，应为最后出场的票往往容易获胜（更早出场的选票互相残杀消耗掉了）
补救办法就是，当找出最后的获胜者之后，再遍历一下原来的数组，对这个元素计数，看看是不是真的数量大于1/2,如果不是，那就是压根
没有一个元素数量多于1/2的。
